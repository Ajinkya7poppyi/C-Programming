// C Program To Convert ASCII Number to Packed BCD
...
void ascii_to_packed_bcd(unsigned char* ascii_text, unsigned char* bcd_text)
{
    ...
    for (size_t i=0; i< ascii_length/2; i++)
    {
        ...
        if(ascii_text[i*2] >= ZERO_CHAR && ascii_text[i*2] <= NINE_CHAR)
        {
            bcd_text[i] = ( ascii_text[i*2] - ZERO_CHAR)  << 4 ;
        }
        ...
        else if (ascii_text[i*2] >= CAPA_CHAR && ascii_text[i*2] <= CAPF_CHAR)
        {
            bcd_text[i] = ( 10 + ascii_text[i*2] - CAPA_CHAR )  << 4 ;
        }
        /*Check if the value lied in a-f range and subtract from 'a' to get decimal value and shift to Most Significant Bits. Add decimal value of a i.e 10*/
        else if (ascii_text[i*2] >= SMALLA_CHAR && ascii_text[i*2] <= SMALLF_CHAR)
        {
            bcd_text[i] = ( 10 + ascii_text[i*2] - SMALLA_CHAR)  << 4 ;
        }

        /*Perform OR operation with previous byte to mix both to get packed BCD*/
        /*Check if the value lied in 0-9 range and subtract from '0' to get decimal value and shift to Most Significant Bits*/
        if(ascii_text[i*2+1] >= ZERO_CHAR && ascii_text[i*2+1] <= NINE_CHAR)  
        {
            bcd_text[i] |= ( ascii_text[i*2+1] - '0'); 
        }
        /*Check if the value lied in A-F range and subtract from 'A' to get decimal value. Add decimal value of a i.e 10*/
        else if (ascii_text[i*2+1] >= CAPA_CHAR && ascii_text[i*2+1] <= CAPF_CHAR)
        {
            bcd_text[i] |= ( 10 + ascii_text[i*2+1] - CAPA_CHAR)   ; 
        }
        /*Check if the value lied in a-f range and subtract from 'A' to get decimal value. Add decimal value of a i.e 10*/
        else if (ascii_text[i*2+1] >= SMALLA_CHAR && ascii_text[i*2+1] <= SMALLF_CHAR) 
        {
            bcd_text[i] |= ( 10 + ascii_text[i*2+1] - SMALLA_CHAR ) ; 
        }
    }
}
